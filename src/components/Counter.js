  ...
  return (
    <div className='widget-counter container'>
      ...
      <div id='count' style={style}>
        Number {count} is {evenOdd ? "even" : "odd"} {/* STEP 3 */}
      </div>
      <div id="count">
        <button id='increment' onClick={event => {setCount(count + 1); setEvenOdd(!evenOdd)}}>Increment</button>
        <button id='decrement' onClick={event => {setCount(count - 1); setEvenOdd(!evenOdd)}}>Decrement</button>
        <button id='resetCount' onClick={event => {setCount(0); setEvenOdd(true)}}>Reset</button>
      </div>
    </div>
  );
}
