...
export default function Moods() {
  ...
  const [feel, setFeel] = useState(initialMood)
  /* STEP 1 */
  ...
  const style = {
    fontSize: '1.5em',
    marginBottom: '0.3em',
    color: feel === happyMood ? "royalblue" : "crimson", /* STEP 2 */
  };

  return (
    <div className='widget-moods container'>
      <h2>Moods</h2>
      <div id='mood' style={style}>{feel}</div> {/* STEP 3 */}
      <div>
        <button id='makeHappy' onClick={event => {setFeel(happyMood)}}>Make Happy</button>
        <button id='makeSad' onClick={event => {setFeel(sadMood)}}>Make Sad</button>
        <button id='resetMood' onClick={event => {setFeel(initialMood)}}>Reset</button>
      </div>
    </div>
  );
}
